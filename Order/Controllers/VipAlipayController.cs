...
{
    public class VipAlipayController : JsonController
    {
        ...
        public string TradePayCallBack()
        {
            try
            {
                Log4NetHelper.Info(log, "=======================VIP TradePayCallBack Executing=======================");
                ...
                if (requstParams != null && requstParams.Count > 0)
                {
                    ...
                    if (signVerfied)
                    {
                        ...
                        { 
                            if (service.VipChargeCoin(order.UserId, order.OrderId, (int)total_amount, order.VipType))
                                return "success";
                            return "failure";
                        }
                        else
