...
{
    public class VipWxPayController : JsonController
    {
        ...
        public void TradePayCallBack()
        {
            lock (objlock)
            {
                Log4NetHelper.Info(log, "=======================VIP Wechat TradePayCallBack Start=======================");
                ...
                if (result)
                    ...
                    text = "invalid sign";
                Log4NetHelper.Info(log, "=======================VIP Wechat TradePayCallBack End=======================");
        ...
        private bool DoProcess()
        {
            try
            {
                ...
                if (md5Str.Equals(sign))
                {
                    ...
                    {
                        if (service.VipChargeCoin(order.UserId, ordersn, (int)order.ChargeMoney, order.VipType))
                            return true;
                        return false;
                    }
                    else
